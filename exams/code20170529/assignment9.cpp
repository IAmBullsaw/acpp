...
public:
  ...
  sort_insert_iterator(Container & c)
    :container_{c}
  {}

  sort_insert_iterator& operator++() { return *this; }
  sort_insert_iterator operator++(int) { return *this; }
  sort_insert_iterator& operator=(typename Container::value_type const& val) {
    auto it = find_if(begin(container_), end(container_), [&val](auto const& a){return a > val;});
...
template <typename Container>
auto sort_inserter(Container & c) {
